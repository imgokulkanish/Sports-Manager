...
- **App name:** Sports Manager
- **Netlify slug:** `sportsmanager-gk` (matches your existing `-gk` convention)
...
- `public/manifest.json` — real schema pulled from Shuttle's original file,
  not guessed. `short_name: "Sports Mgr"` fits the ~12-char home-screen label
...
- `src/shell/theme.jsx` — localStorage key renamed `shuttle-manager:theme`
  → `sportsmanager:theme`. One-time, harmless: everyone's saved light/dark
...
2. **Icon PNGs.** `manifest.json` and `index.html` still point at
   ...
   whether "Sports Manager" gets its own mark, or keeps a shuttle/ball motif).
3. **`package.json` name field** — whenever you scaffold the merged repo,
   its `package.json` should say `"name": "sports-manager"` rather than
